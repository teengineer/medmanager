...
function AboutPage() {
  ...
  return (
    <main className="mx-auto max-w-xl p-6">
      <Link
        to="/"
        className="inline-flex items-center gap-1 text-sm text-mute hover:text-brand"
      >
      ...
      <p className="mt-6 text-ink-soft">{t("about.description")}</p>
      ...
      <section className="surface-card mt-8 p-5">
        ...
        <a
          ...
          className="flex items-center gap-3 rounded-field border border-line px-4 py-3 transition hover:border-brand hover:bg-brand-light"
        >
          <svg viewBox="0 0 24 24" className="size-6 text-ink" aria-hidden>
            <path
              fill="currentColor"
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-mute">{GITHUB_URL.replace("https://", "")}</p>
          </div>
